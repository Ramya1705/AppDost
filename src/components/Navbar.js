  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all ${
        isScrolled
          ? 'bg-white/90 dark:bg-slate-950/90 backdrop-blur-2xl border-b-2 border-slate-200 dark:border-slate-800 shadow-2xl'
          : 'bg-transparent'
      }`}
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((item) => (
              <a
                key={item}
                href={navId(item)}
                onClick={(e) => handleNavClick(e, navId(item))}
          ...
          <div className="px-6 py-6 space-y-4">
            {navLinks.map((item) => (
              <a
                ...
                href={navId(item)}
                onClick={(e) => handleNavClick(e, navId(item))}
                className="block py-3 px-4 rounded-xl hover:bg-purple-500/10 font-semibold"
              >
                {item}
              </a>
            ))}
